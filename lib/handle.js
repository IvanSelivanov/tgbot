...
export async function handleUpdate(update) {
  ...
  if (!allowedUserIds().includes(from.id)) return;
...
async function handleVoice(chatId, messageId, voice) {
  ...
  const result = await withTyping(chatId, async () => {
    ...
    const audio = await Telegram.download(file.file_path);
    return transcribe(audio, { mimeType: voice.mime_type ?? "audio/ogg" });
  });
...
async function notifyFailure(chatId, messageId, error) {
  ...
  if (error instanceof RateLimited) {
    ...
    text = `Telegram не отдал файл: ${error.message}`;
  } else {
    text = `Что-то сломалось: ${error.message}`;
  }
